...
def make_transaction(log_obj,account_data,tran_type,amount,**others):
    ...
    if tran_type in  settings.TRANSACTION_TYPE:
        ...
        if tran_type == 'transfer':
            transfer_id = others['transfer_acct']
            transfer_data = accounts.load_current_balance(transfer_id)
            transfer_data['balance'] += amount
            accounts.dump_account(transfer_data)
            log_obj.info("account:%s   action:%s    amount:%s" %
                          (transfer_id, tran_type, '+' + str(amount)) )
        return account_data
    else:
