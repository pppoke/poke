...
def transfer(acc_data):
    ...
    back_flag = False
    while not back_flag:
        transfer_acct = input("\033[33;1mInput withdraw acct:\033[0m").strip()
        if not auth.acc_exist(transfer_acct, access_logger):
            if transfer_acct == 'b':
                back_flag = True
            else:
                print("该账户不存在，无法转账！")
            continue
        transfer_amount = input("\033[33;1mInput withdraw amount:\033[0m").strip()
        if len(transfer_amount) > 0 and transfer_amount.isdigit():
            new_balance = transaction.make_transaction(trans_logger, account_data, 'transfer', transfer_amount, transfer_acct=transfer_acct)
            if new_balance:
                print('''\033[42;1mNew Balance:%s\033[0m''' % (new_balance['balance']))
        else:
