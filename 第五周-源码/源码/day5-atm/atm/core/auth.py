...
def acc_exist(user_id,log_obj):
    '''
    account auth exist func
    :user_id: acct id , only saves in memory
    :return:
    '''
    db_path = db_handler.db_handler(settings.DATABASE)
    account_file = "%s/%s.json" % (db_path, user_id)
    if not os.path.isfile(account_file):
        log_obj.error("account [%s] is not exist" % user_id)
        return False
    log_obj.info("account [%s] is exist" % user_id)
    return True
